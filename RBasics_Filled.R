...
?round
# The help page will open in the bottom right pane
...
# Note the read text produced when library(dplyr) is loaded.
# "The following objects are masked from ‘package:stats’:
#   filter, lag"
# This means the stats and dplyr package have functions with the same names
# but not necessarily the same functionality
# We can specify which function we wants by specifying the package then ::
ts <- 1:100
stats::filter(ts, rep(1, 3)) # proper use of stats::filter
dplyr::filter(ts, rep(1, 3)) # wrong use of dplyr::filter, returns an error

head(iris)
stats::filter(iris, Sepal.Length > 6) # proper use of stats::filter
dplyr::filter(iris, Sepal.Length > 7) # wrong use of dplyr::filter, returns an error

# If the package is not specified, the last package loaded 
filter(ts, rep(1, 3)) # proper for stats package
filter(iris, Sepal.Length > 7) # proper for dplyr package
...
install.packages("beepr") # or by package pane
library(beepr) # or by package pane
...
# Data structures will be exemplified by built-in R data sets
...
# to generate descriptive statistics for each numeric column
summary(Ptbo_Temps)
